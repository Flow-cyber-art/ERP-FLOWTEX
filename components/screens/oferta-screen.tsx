...
export function OfertaScreen({ profile }: { profile: Profile }) {
  ...
  // Podgląd/PDF oferty — jeden, realny plik PDF: strona tytułowa +
  // TABELA CENOWA + warunki dorysowane przez pdf-lib, a dla każdej
  // wybranej karty z podpiętym plikiem w Storage (patrz docPaths wyżej
  // — technology_documents/097/098) doklejane są 1:1 strony
  // PRAWDZIWEGO pliku PDF karty technicznej — to jest ustalenie z
  // rozmowy: karta wybrana w kroku 2 ma trafić DO oferty, nie być tylko
  // linkiem do otwarcia osobno. Dla kart bez podpiętego pliku, ale z
  // treścią w bazie (description/workPhases/investorBenefits — patrz
  // 099_faza0_tresc_dokumentu_oferty.sql) rysowana jest karta zastępcza.
    ...
    try {
      const items: OfferPdfItem[] = selectedTechnologies.map((tech) => {
        ...
        return {
          ...
          fetchRealPdf: storagePath
            ? async () => {
                const url = await getTechnologyPdfSignedUrl(storagePath);
      ...
      const customItemsInput: OfferPdfCustomItem[] = customItems.map((it) => ({
        name: it.name,
        unit: it.unit,
        qty: it.qty,
      ...
      const bytes = await buildOfferPdf({
        ...
        subtotalLabel: formatPLN(subtotal),
        discountPercent: num(discountPercent),
        discountLabel: formatPLN(discountAmount),
        totalLabel: formatPLN(total),
      });
          ...
          <>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 16 }}>
              ...
              <View style={{ flexDirection: "row", gap: 10 }}>
                <OButton
                  label={exportingPdf ? "Generowanie PDF..." : "Podgląd / PDF"}
                  secondary
                  disabled={exportingPdf || (selectedTechnologies.length === 0 && customItems.length === 0)}
