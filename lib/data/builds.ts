...
export type BuildStageCompletionRow = {
  buildId: number;
  stageName: string;
  completedAt: string;
...
export async function listBuildStageCompletions(): Promise<BuildStageCompletionRow[]> {
  const { data, error } = await supabase
    .from("build_stage_completions")
    .select("build_id, stage_name, completed_at, completed_by");
  if (error) throw new Error(error.message);
  return (data ?? []).map((row) => ({
...
export async function setBuildStageCompleted(
  buildId: number,
  stageName: string,
  completed: boolean,
  completedBy?: string | null,
  ...
  if (completed) {
    const { error } = await supabase
      .from("build_stage_completions")
      .upsert(
        { build_id: buildId, stage_name: stageName, completed_by: completedBy ?? null },
        { onConflict: "build_id,stage_name" },
      );
    if (error) throw new Error(error.message);
  } else {
    const { error } = await supabase
