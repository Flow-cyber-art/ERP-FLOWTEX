-- ------------------------------------------------------------
-- Archiwizacja materiałów zamiast twardego usuwania.
--
-- Ustalenie: materiału ze stanem zero (albo śmieciowego wpisu testowego)
-- nie usuwamy z kartoteki — historia (partie, zamówienia, raporty,
-- rozliczenia zamkniętych budów) go referencjuje po ID, więc usunięcie
-- albo złamałoby te powiązania, albo baza by na to nie pozwoliła. Zamiast
-- tego materiał dostaje flagę "active" — domyślny widok Magazynu (i
-- ekran "+ Nowy materiał"/"Zamów materiał spoza listy") mają go dalej
-- podpowiadać przy dopasowaniu nazwy (żeby nie powstał duplikat indeksu
-- ani nazwy), ale znika z domyślnej listy do przeglądania.
--
-- Materiały faktycznie bez żadnej historii (testowe/śmieciowe wpisy)
-- można nadal skasować ręcznie w Supabase — o tym admin zdecydował sam,
-- poza aplikacją.
-- ------------------------------------------------------------

alter table materials add column if not exists active boolean not null default true;

-- Dostawa dotarła dla materiału, który był zarchiwizowany (bo miał zerowy
-- stan) — świeża partia oznacza, że znowu jest w użyciu, więc wraca sam,
-- bez ręcznego "odarchiwizowania" przez Admina. Dotyczy obu ścieżek
-- przyjęcia dostawy (receive_order — orders/order_items, Faza 3/4/5;
-- receive_material_order — starszy, wolny flow material_orders).
-- Reszta ciała funkcji identyczna jak w 037_dopasowanie_materialu_odbior.sql.
