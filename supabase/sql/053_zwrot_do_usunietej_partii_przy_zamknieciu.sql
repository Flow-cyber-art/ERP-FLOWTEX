-- ============================================================
-- Fix: close_build (043_fix_close_build_podwojne_odjecie_partii.sql)
-- blokował rozliczenie budowy twardym błędem "Nie można zwrócić materiału
-- #% na magazyn — partia źródłowa już nie istnieje (skonsumowana gdzie
-- indziej)", gdy Admin wybrał "zwrot na magazyn" dla pozycji, której
-- oryginalna partia (material_batches) została w międzyczasie skasowana
-- (ON DELETE SET NULL po wyczerpaniu jej do zera gdzie indziej) — czyli w
-- praktyce najczęstszy przypadek, nie rzadki wyjątek (ten sam problem co
-- w unassign_material_from_build, naprawiony wcześniej w
-- 028_fix_zwrot_do_usunietej_partii.sql). Admin nie miał wyjścia poza
-- ręczną zmianą decyzji na "Do wyrzucenia", co fałszywie księgowało
-- odzyskany materiał jako stratę.
--
-- Poprawka: identyczna zasada jak w 028 — gdy partia źródłowa już nie
-- istnieje, zwrot tworzy NOWĄ partię (ta sama ilość i cena, po jakiej
-- materiał faktycznie siedział na budowie) zamiast doliczać do
-- nieistniejącego wiersza. Reszta funkcji identyczna jak w 043.
--
-- Uruchom PO 043_fix_close_build_podwojne_odjecie_partii.sql. Bezpieczne
...
begin
  ...
  loop
    ...
    -- "sourceBatchId" tego lota jest już null. Dawniej: twardy błąd
    -- ("Wybierz Do wyrzucenia"). Teraz: zwrot tworzy nową partię niżej,
    -- więc TU nie ma już powodu blokować — usunięty wcześniejszy
    -- `raise exception` dla (v_decision = 'zwrot' and v_batch_id is null).

    -- Dopasowanie dokładnie JEDNEGO wiersza (patrz komentarz w 043) — gdy
    -- do tej samej budowy/materiału/partii źródłowej istnieje więcej niż
    -- jeden lot (np. dwa osobne przypisania tej samej partii), bierzemy
    -- zawsze ten sam, deterministyczny wiersz zamiast modyfikować
    ...
    if v_decision = 'zwrot' then
      if v_batch_id is not null then
        ...
        -- Oryginalna partia już nie istnieje — zwrot trafia do nowej
        -- partii (ta sama ilość i cena) zamiast do nieistniejącego wiersza,
        -- jak w 028_fix_zwrot_do_usunietej_partii.sql.
