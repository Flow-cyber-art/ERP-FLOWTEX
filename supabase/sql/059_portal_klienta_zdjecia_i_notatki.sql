-- ============================================================
-- Portal Klienta: przełączniki "Udostępnij zdjęcia klientowi" i
-- "Udostępnij notatki klientowi" (Admin, per budowa — ten sam wzorzec co
-- istniejące show_contract_value_to_client), plus rozszerzenie
-- get_public_build o realne dane zamiast placeholderów:
--   - technologyName — nazwa przypisanej technologii (build_technology_
--     snapshot), do karty "Zastosowana technologia";
--   - photos — id i data zdjęć z build_photos (WYŁĄCZNIE gdy
--     show_photos_to_client = true), do galerii. Miniaturka budowana po
--     stronie frontu z driveFileId (foldery na Drive mają już "anyone
--     with the link: reader" ustawione przy tworzeniu, patrz
--     supabase/functions/drive-photos/index.ts findOrCreateFolder), więc
--     nie trzeba tu żadnych dodatkowych uprawnień;
--   - notes — data + treść notatki brygadzisty z raportów zatwierdzonych
--     (WYŁĄCZNIE gdy show_notes_to_client = true), do listy "Ostatnie
--     aktualizacje".
--
-- Domyślnie OBA przełączniki wyłączone (false) — Admin decyduje per
-- budowa, czy klient widzi zdjęcia/notatki, tak jak już decyduje o
-- widoczności wartości kontraktu.
--
-- Uruchom PO 057_portal_klienta_etapy_z_materialow.sql. Bezpieczne do
-- wielokrotnego wklejenia. Jak uruchomić: Supabase Dashboard -> SQL
-- Editor -> wklej całość -> Run.
-- ============================================================

alter table builds add column if not exists show_photos_to_client boolean not null default false;
alter table builds add column if not exists show_notes_to_client boolean not null default false;
...
begin
  ...
  -- Etapy technologii — planowany koszt etapu (build_material_plan ×
  -- aktualna cena materiału) vs zużyty koszt etapu (suma realnego kosztu
  -- FIFO z report_materials oznaczonych tym stage_name).
  select json_agg(
    json_build_object(
      'name', s.stage_name,
      'percent', case
