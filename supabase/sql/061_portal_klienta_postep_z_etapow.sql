-- ============================================================
-- Portal Klienta: ogólny procent budowy (gauge) miał liczyć się z
-- CAŁOŚCI technologii — czyli ze średniej procentów wszystkich etapów
-- (patrz 060_portal_klienta_procent_etapu_bez_linku.sql) — zamiast z
-- surowej wartości zużytych/zaplanowanych materiałów budowy (build_
-- materials), która miesza w to też materiały "dodatkowe" (spoza planu
-- technologii, patrz builds-screen.tsx "Materiały dodatkowe") i przez to
-- nie odpowiada 1:1 postępowi etapów widocznemu niżej na tej samej
-- stronie.
--
-- Priorytet liczenia postępu głównego gauge'a:
--   1. są etapy technologii (v_stages niepuste) -> średnia ich procentów,
--   2. brak etapów, ale są materiały budowy (build_materials, np. budowa
--      bez przypisanej technologii, tylko z materiałami "dodatkowymi")
--      -> stary wzór wartościowy (zużyty/zaplanowany koszt),
--   3. zupełny brak planu materiałowego -> fallback czasowy (dni od
--      startu / czas trwania), jak dotąd.
--
-- Przy okazji: zdjęcia w karcie "Zdjęcia z budowy" pokazywały do 24
-- najnowszych zdjęć z CAŁEJ budowy naraz — przy dłuższej budowie to
-- szybko rośnie do kilkudziesięciu/kilkuset zdjęć w jednej siatce. Teraz
-- portal pokazuje zdjęcia WYŁĄCZNIE z najnowszego dnia, w którym cokolwiek
-- wgrano (nadal do 24 sztuk z tego dnia, w praktyce nigdy tyle nie będzie).
--
-- Uruchom PO 060_portal_klienta_procent_etapu_bez_linku.sql. Bezpieczne
...
begin
  ...
  if v_build.show_photos_to_client then
    ...
    into v_photos
    from (
      select * from build_photos
      where "buildId" = v_build.id
        and "createdAt"::date = (
          select max("createdAt"::date) from build_photos where "buildId" = v_build.id
        )
