-- ============================================================
-- Portal Klienta: notatki AI zamiast surowych notatek brygadzisty.
--
-- Kontekst: `reports.note` to notatka wpisywana przez brygadzistę pod
-- presją czasu na końcu dnia — bywa skrótowa, emocjonalna albo wspomina
-- rzeczy, których zleceniodawca nie powinien czytać wprost. Admin nadal
-- widzi ten surowy tekst (ReportCard) — to jest jego robocze narzędzie.
-- Klient NIGDY nie widzi `reports.note` wprost. Dwa niezależne
...
-- Functions (supabase/functions/generate-report-note,
-- supabase/functions/generate-client-summary).
--
-- Uruchom PO 062_portal_klienta_galeria_wszystkich_dni.sql. Bezpieczne
-- do wielokrotnego wklejenia. Jak uruchomić: Supabase Dashboard -> SQL
-- Editor -> wklej całość -> Run.
-- ============================================================

alter table reports add column if not exists client_note text;
alter table reports add column if not exists client_note_generated_at timestamptz;

alter table builds add column if not exists ai_client_summary text;
alter table builds add column if not exists ai_client_summary_generated_at timestamptz;
