-- ============================================================
-- Portal Klienta: postęp i procent etapów liczone WYŁĄCZNIE z
-- zatwierdzonych raportów.
--
-- Błąd: `v_used_cost` (postęp ogólny) liczył się z build_materials.used —
-- licznika aktualizowanego natychmiast przy WYSŁANIU raportu przez
-- brygadzistę, zanim Admin go w ogóle zobaczył, a procent każdego etapu
-- (`v_stages`) sumował report_materials bez filtra na status raportu.
-- Efekt: klient widział niezerowy postęp budowy, mimo że żaden raport nie
-- był jeszcze zatwierdzony — dokładnie odwrotnie niż reszta portalu
-- (zdjęcia/notatki), gdzie "zatwierdzone" jest już wszędzie warunkiem.
--
-- Poprawka liczy `v_used_cost` i `v_stages` wprost z report_materials
-- JOIN reports WHERE status = 'approved' (zamiast z żywego licznika
-- build_materials.used), a `v_days_elapsed`/`v_last_update` (oś czasu i
-- "Ostatnia aktualizacja") też patrzą tylko na zatwierdzone raporty.
--
-- Uruchom PO 065_stan_minimalny_nowego_materialu.sql (kolejność
-- niezależna, dowolna). Bezpieczne do wielokrotnego wklejenia. Jak
...
begin
  ...
  -- osi czasu klienta — raport dopiero wysłany (submitted) jeszcze może
  -- zostać poprawiony/odrzucony przez Admina.
  select count(distinct r.date) into v_days_elapsed
  from reports r
  where r."buildId" = v_build.id and r.status = 'approved';

  select coalesce(sum(bm.planned * bm."unitPrice"), 0)
    into v_planned_cost
    from build_materials bm
    where bm."buildId" = v_build.id;

  select coalesce(sum(rm.cost), 0) into v_used_cost
  ...
  left join (
    ...
    from report_materials rm
    join reports r on r.id = rm."reportId"
    where r."buildId" = v_build.id and rm.stage_name is not null and r.status = 'approved'
