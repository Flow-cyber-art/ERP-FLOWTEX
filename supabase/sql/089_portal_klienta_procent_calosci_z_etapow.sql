-- ============================================================
-- Portal Klienta: "% całości zlecenia" rozjeżdżał się z listą etapów.
--
-- Zgłoszone: brygadzista odznaczył "Gruntowanie" jako zakończone
-- (checkbox z 071_zakonczenie_etapu_checkbox.sql) — na liście etapów
-- poprawnie pokazywało 100%, ale duży pierścień u góry ("X% całości
-- zlecenia") dalej pokazywał 0%. Powód: get_public_build liczył
-- `v_progress` (całość) z SUROWEGO zużycia materiału względem planu
-- (build_materials / report_materials), zupełnie osobno od `v_stages`
-- (etapy) — który OD 071 honoruje ręczne zakończenie etapu (checkbox =
-- zawsze 100%, niezależnie od realnego zużycia). Dwa niezależne
-- wyliczenia z dwóch różnych źródeł prawdy, więc mogły (i potrafiły)
-- pokazać sprzeczne liczby: etap "zakończony", a całość budowy dalej 0%.
--
-- Naprawa: całość liczona jako ŚREDNIA z już policzonych procentów
-- etapów (v_stages) zamiast osobnego zapytania po build_materials —
-- jedno źródło prawdy, więc pierścień całości i lista etapów zawsze
-- się zgadzają. Fallback (brak etapów z planu materiałowego — budowa
-- bez wgranej technologii) zostaje jak było: proporcja dni roboczych.
--
