-- ============================================================
-- Procent etapu w Portalu Klienta ważony ILOŚCIĄ PLANU, nie liczbą
-- pozycji materiałowych.
--
-- Zgłoszone na budowie "Żłobek Miejski" (id 1): etap "Warstwa
-- zamykająca" pokazywał 25%, mimo że główny materiał tego etapu
-- (żywica, plan 360 kg) nie był w ogóle ruszony. Przyczyna: dotychczasowy
-- wzór (070/071) liczył PROSTĄ średnią z osobno policzonych procentów
-- KAŻDEJ pozycji materiałowej w etapie:
--
--   avg( least(zużyto_i / plan_i, 1) )  po wszystkich materiałach i etapu
--
-- W tym etapie są 4 pozycje: żywica (plan 360 kg), utwardzacz (7,2 kg),
-- przyspieszacz (1,08 kg) i dodatek TopCoat (plan tylko 1,8 kg). Ktoś
-- zapisał w raporcie 4 kg dodatku TopCoat (ponad dwukrotność JEGO planu,
-- więc ta jedna pozycja ucina się na 100%) — a każda pozycja liczy się w
-- średniej TAK SAMO, niezależnie od tego, czy to 1,8 kg dodatku, czy
-- 360 kg głównej żywicy. Stąd (0+0+0+100)/4 = 25%, mimo że fizycznie
-- prawie nic z tego etapu nie zostało zrobione.
--
-- NAPRAWIONE (i celowo, na stałe — nie cofać do prostej średniej):
-- procent etapu liczy się teraz jako SUMA zużycia (uciętego do planu na
-- KAŻDEJ pozycji z osobna, żeby nadwyżka na jednym materiale nie
-- "dopłacała" reszcie) podzielona przez SUMĘ planu całego etapu:
--
--   sum( least(zużyto_i, plan_i) ) / sum(plan_i) * 100
--
-- To naturalnie waży materiał jego udziałem w planie etapu (360 kg
-- żywicy waży ~316x więcej niż 1,8 kg dodatku) zamiast traktować każdą
-- pozycję jak "1 głos" w średniej. Ma sens tylko dlatego, że materiały
-- w jednym etapie tej samej technologii są w tej samej jednostce (kg) —
-- jeśli kiedyś etap miałby mieszane jednostki (kg + szt.), ta metoda by
-- się nie sprawdziła i trzeba by wrócić do ważenia wartością (ilość ×
-- cena jednostkowa z `materials.unitPrice`), nie z powrotem do prostej
-- średniej po pozycjach — TA jest źródłem błędu z tego zgłoszenia.
--
-- Ręczne zakończenie etapu (checkbox, build_stage_status) nadal
-- nadpisuje etap na 100% bez względu na powyższe — bez zmian względem
-- 071_zakonczenie_etapu_checkbox.sql.
--
-- Uruchom PO 089_portal_klienta_procent_calosci_z_etapow.sql. Bezpieczne
...
begin
  ...
  -- jako zakończony (build_stage_status), inaczej suma zużycia (uciętego
  -- do planu NA KAŻDEJ pozycji) / suma planu całego etapu — patrz duży
  -- komentarz na górze pliku, dlaczego NIE prosta średnia po pozycjach.
  select
  ...
  if v_progress is not null then
    ...
    -- Budowa bez etapów z planu materiałowego (brak wgranej technologii)
    -- — jedyny sensowny szacunek to proporcja dni roboczych, jak dotąd.
